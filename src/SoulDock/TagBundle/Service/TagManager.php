...
namespace SoulDock\TagBundle\Service;

use Doctrine\ORM\EntityManager;
use Doctrine\ORM\Query\Expr;
use Doctrine\Common\Collections\ArrayCollection;
use SoulDock\TagBundle\Entity\Tag;
use SoulDock\TagBundle\Entity\TaggedEntity;
use SoulDock\TagBundle\Entity\Taggable;

/**
 * Class TagManager
 *
 * @package SoulDock\TagBundle\Service
 */
class TagManager
{
    /**
     ...
     * @param string|null   $taggedEntityClass
     */
    public function __construct(EntityManager $em, $tagClass = null, $taggedEntityClass = null)
    {
        $this->em = $em;
        $this->tagClass = $tagClass ?: 'SoulDock\TagBundle\Entity\Tag';
        $this->taggedEntityClass = $taggedEntityClass ?: 'SoulDock\TagBundle\Entity\TaggedEntity';
    }
    ...
    public function saveTagging(Taggable $resource)
    {
        ...
        if ($oldTags !== null and is_array($oldTags) and !empty($oldTags)) {
            ...
            if (sizeof($tagsToRemove)) {
                $builder = $this->em->createQueryBuilder();
                $builder
                    ->delete($this->taggedEntityClass, 't')
                    ->where('t.tag_id')
    ...
    protected function getTagging(Taggable $resource)
    {
        return $this->em
            ...
            ->select('t')
            ->from($this->tagClass, 't')
            ->innerJoin('t.entities', 't2', Expr\Join::WITH, 't2.resourceId = :id AND t2.resourceType = :type')
    ...
    /**
     * Creates a new TaggedEntity object
     *
     * @param Tag      $tag      Tag object
    ...
    {
        $taggedEntity = new $this->taggedEntityClass();
        $taggedEntity->setTag($tag);
        $taggedEntity->setResourceType($resource->getTaggableType());
        $taggedEntity->setResourceId($resource->getTaggableId());
